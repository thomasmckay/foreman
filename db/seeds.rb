...
## TODO remove
## To find current assignable permissions
#perms_by_block = Role.last.setable_permissions.group_by { |p| p.security_block.to_s }
#perms_by_block.keys.sort.each do |block|
#  perms_by_block[block].each do |permission|
#    begin
#      klass_string = block.singularize.camelize
#      klass        = klass_string.constantize
#      puts %Q(Permission.find_or_create_by_resource_type_and_name #{klass}, "#{permission.name}")
#    rescue NameError => e
#      Rails.logger.debug "Unknown resource type #{klass_string}, creating unlimited Permission (#{e.message})"
#      puts %Q(Permission.find_or_create_by_resource_type_and_name nil, "#{permission.name}")
#    end
#  end
#end
...
permissions = [
    ...
    # Audit comes from 'plugin', 'maybe we could hack scope_search inside by reopening class?'
    [nil, 'view_audit_logs'],
    ...
    # VMs come from fog 'plugin', 'maybe we could hack scope_search inside by reopening classes?'
    [nil, 'view_compute_resources_vms'],
    [nil, 'create_compute_resources_vms'],
    [nil, 'edit_compute_resources_vms'],
    [nil, 'destroy_compute_resources_vms'],
    [nil, 'power_compute_resources_vms'],
    [nil, 'console_compute_resources_vms'],
